...
## **2.4 Conclusiones Six Hump Camel Function:**
...
## **Conclusión general primera parte: Optimización númerica (Griewank function & Six Hump Camel function en conjunto)**

Unificando las conclusiones de ambas funciones se llegó a la conclusión general en la que:

### **Usando el método por descenso por gradiente:**


El método de descenso por gradiente se destacó por su alta eficiencia computacional: con un bajo número de evaluaciones logró converger rápidamente. Sin embargo, su rendimiento se vio afectado por la naturaleza multimodal de ambas funciones, especialmente en Griewank, donde se quedó atrapado en mínimos locales alejados del óptimo global. En el caso de la función Six Hump Camel, en 2D logró alcanzar uno de los óptimos, pero en 3D volvió a presentar dificultades para escapar de pozos locales. Esto refuerza la conclusión de que el descenso por gradiente explota bien la información local, pero carece de mecanismos de exploración global. En resumen, se determina que existe eficiencia local pero exploración global limitada.


"La técnica de optimización por descenso de gradiente generalmente no es viable para datos de mayor dimensión, y es aquí donde se pueden considerar técnicas de optimización alternativas para el descenso de gradiente, con el fin de aumentar la optimización. Estas técnicas ayudan al modelo a converger a la solución óptima con un número mínimo de hiperparámetros y con un número mínimo de pasos a realizar por los gradientes." (Analytics India Magazine, 2021)


### **Usando Métodos heurísticos:**

Los métodos heurísticos demostraron una **mejor capacidad exploratoria** en ambas funciones:

- **Algoritmo Genético (GA)** logró una muy buena aproximación al óptimo global en ambos casos. En la función Griewank, alcanzó valores cercanos a cero con poblaciones grandes y muchas generaciones; en la función de las seis jorobas de camello, fue capaz de encontrar un mínimo global rápidamente, aunque no siempre identificó ambos óptimos simultáneamente.

- **PSO (Particle Swarm Optimization)** mostró resultados altamente consistentes. En Griewank alcanzó valores óptimos con menos evaluaciones que GA, y en la función de las seis jorobas logró converger al valor objetivo en todas las pruebas, aunque solo a uno de los mínimos globales. El uso de un coeficiente de inercia adecuado (e.g., 0.5) fue clave para balancear exploración y explotación.

- **Evolución Diferencial (DE)** ofreció un excelente compromiso entre eficiencia y exploración. Aunque en Griewank su rendimiento fue inferior al de GA y PSO, en la función Six Hump Camel se posicionó como el **único algoritmo** que encontró **ambos mínimos globales** de forma consistente en 2D y 3D, evitando caer en mínimos locales.


En resumen, el uso de métodos heurísticos para ambas funciones demostró una exploración efectiva con respecto a sus óptimos globales pero con un mayor costo computacional.


### **Reflexión final del uso de estos métodos:**

La comparación evidencia que no existe un algoritmo universalmente superior a otro, cada uno tiene fortalezas particulares según el problema evaluado. El descenso por gradiente es útil cuando se dispone de información local precisa y funciones suaves, mientras que los algoritmos heurísticos ofrecen una alternativa más robusta para problemas con múltiples óptimos y estructuras complejas en el espacio de búsqueda. En contextos reales donde la función objetivo puede ser no diferenciable o ruidosa, los métodos heurísticos se perfilan como herramientas más versátiles, aunque a costa de mayor tiempo de calculo/computo de los algoritmos.
...
```{r mapa_Colombia, echo=FALSE,warning=FALSE,message=FALSE}
...
# Cerrar la ruta, agregando la conexión de la última ciudad con la primera
recorrido_optimizado_cerrado <- c(recorrido_optimizado, recorrido_optimizado[1])

# Mostrar la ruta cerrada
print("Ruta cerrada:")
print(recorrido_optimizado_cerrado)
```
Gráfico

```{r plot_Hormigas, echo=FALSE,warning=FALSE,message=FALSE}
library(maps)
...
```{r gif_Hormiga, echo=FALSE,warning=FALSE,message=FALSE}
# Cargar librerías necesarias
library(ggplot2)
library(gganimate)
library(ggmap)
...
algoritmo_genetico <- function() {
  ...
  historial_costos <- list()  # Crear una lista para almacenar los costos en cada generación
  ...
  for (generacion in 1:num_generaciones) {
    ...
    # Guardar los costos de la población actual
    costos <- sapply(poblacion, calcular_costo)
    historial_costos[[generacion]] <- costos  # Guardar el vector de costos para la generación actual
  ...
  # Obtener el costo mínimo final
  costos_finales <- unlist(historial_costos)
  min_costo <- min(costos_finales)  # Mejor costo encontrado
  
  # Obtener la ruta correspondiente al costo mínimo
  mejor_indice <- which.min(costos_finales)
  generacion_mejor <- (mejor_indice - 1) %/% tamano_poblacion + 1  # Generación donde se encontró el mejor costo
  poblacion_mejor <- poblacion[[mejor_indice %% tamano_poblacion]]
  
  return(list(mejor_ruta = poblacion_mejor, mejor_costo = min_costo, generacion = generacion_mejor))
}

# Ejecutar el algoritmo genético
resultado <- algoritmo_genetico()

# Mostrar la mejor ruta y el costo encontrado
cat("Mejor ruta encontrada:", resultado$mejor_ruta, "\n")
cat("Costo total de la ruta:", resultado$mejor_costo, "\n")
cat("Generación en la que se encontró el mejor costo:", resultado$generacion, "\n")


```

```{r plot_EA, echo=FALSE,warning=FALSE,message=FALSE}
library(maps)
...
# Especificar la ruta según el orden de ciudades
ruta <- resultado$mejor_ruta  # Orden de ciudades según la ruta
...
text(coordenadas$Longitud[ruta], coordenadas$Latitud[ruta], labels=coordenadas$Ciudad[ruta], pos=4, cex=0.8)
```
```{r gif_EA, echo=FALSE,warning=FALSE,message=FALSE}
# Cargar librerías necesarias
library(ggplot2)
library(gganimate)
library(ggmap)
...
# Supongamos que 'mejor_ruta' es la ruta más corta obtenida por el algoritmo genético
mejor_ruta <- resultado$mejor_ruta  # Esta es solo una ruta de ejemplo
...
14. Loshchilov, I. & Hutter, F. (2016). SGDR: Stochastic Gradient Descent with Warm Restarts. arXiv:1608.03983.
15. Shi, Y. & Eberhart, R. (1998). A modified particle swarm optimizer. Proceedings of the IEEE International Conference on Evolutionary Computation.
16. Price, K., Storn, R. & Lampinen, J. (2005). Differential Evolution: A Practical Approach to Global Optimization. Springer.
17. Analytics India Magazine. (2021, 8 de abril). Top alternatives to gradient descent. https://analyticsindiamag.com/top-alternatives-to-gradient-descent/
