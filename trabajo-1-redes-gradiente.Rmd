...
# Calcular valores de la función para toda la trayectoria
f_values <- sapply(path, griewank)
# Encontrar la posición del valor mínimo en el vector f_values
pos_min <- which.min(f_values)
# Calcular la iteración correspondiente (restamos 1 por el punto inicial)
iteracion_optima <- pos_min - 1
# Obtener el punto y valor óptimo encontrado
x_optimo <- path[[pos_min]]
f_optimo <- f_values[pos_min]
...
resultado <- tibble(
  ...
  `Óptimo Encontrado` = paste0(
    "Iteración ", iteracion_optima, ": (", 
    round(x_optimo[1], 4), ", ", 
    round(x_optimo[2], 4), 
    ") con valor ", round(f_optimo, 4)
  )
)
