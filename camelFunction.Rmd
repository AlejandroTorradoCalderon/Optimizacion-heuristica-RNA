...
# --- Función Six-Hump Camel en 2D (se usa para generar la superficie 3D) ---
six_hump_camel_2d <- function(x, y) {
  (4 - 2.1 * x^2 + (x^4)/3) * x^2 + x * y + (-4 + 4 * y^2) * y^2
}

# (Se asume que ya corriste el algoritmo GA y que 'population_history_2d' y 'pop_data_2d' existen)

# Cargar la librería plotly para la visualización 3D
library(plotly)

# Crear una malla para la superficie del Six-Hump Camel
x_seq <- seq(-3, 3, length.out = 100)
y_seq <- seq(-2, 2, length.out = 100)
...
# (Opcional) Convertir el atributo 'generation' a variable numérica, si se requiere
pop_data_2d$generation <- as.numeric(as.character(pop_data_2d$generation))
...
fig  # Visualizar la animación interactiva
```
